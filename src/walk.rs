...
fn walk_match_until_limit_no_label(initial_dirs: &mut Vec<std::path::PathBuf>, limit: usize, match_rx: Regex, match_exact: Option<&String>) -> std::io::Result<(Vec<PathBuf>, Vec<FoundFile>)> {
    let mut dir_q: Vec<PathBuf> = std::mem::take(initial_dirs);
    let mut matches: Vec<FoundFile>;
...
fn walk_match_until_limit_label(initial_dirs: &mut Vec<std::path::PathBuf>, limit: usize, label_pos: i8, match_rx: Regex, match_exact: Option<&String>) -> std::io::Result<(Vec<PathBuf>, Vec<FoundFile>)> {
    let mut dir_q: Vec<PathBuf> = std::mem::take(initial_dirs);
    let mut matches: Vec<FoundFile>;
    let mut match_exact_fn = Some(OsStr::new(""));
    if match_exact.is_some() {
