...
pub fn walk_match_until_limit(initial_dirs: &mut Vec<std::path::PathBuf>, limit: usize, label_pos: i8, match_rx: Regex, match_exact: Option<&OsStr>) -> std::io::Result<(Vec<PathBuf>, Vec<FoundFile>)> {
    if label_pos == 0 {
        return walk_match_until_limit_no_label(initial_dirs, limit, match_rx, match_exact);
    }
    return walk_match_until_limit_label(initial_dirs, limit, label_pos, match_rx, match_exact);
}

fn walk_match_until_limit_no_label(initial_dirs: &mut Vec<std::path::PathBuf>, limit: usize, match_rx: Regex, match_exact: Option<&OsStr>) -> std::io::Result<(Vec<PathBuf>, Vec<FoundFile>)> {
...
fn walk_match_until_limit_label(initial_dirs: &mut Vec<std::path::PathBuf>, limit: usize, label_pos: i8, match_rx: Regex, match_exact: Option<&OsStr>) -> std::io::Result<(Vec<PathBuf>, Vec<FoundFile>)> {
    let mut dir_q: Vec<PathBuf> = std::mem::take(initial_dirs);
    let mut matches: Vec<FoundFile>;
